...
public class WarningCircle extends AbstractGameObject {
    ...
    public void update(float deltaTime) {
        ...
        if (lifeTime >= timeToLive) {
            ...
            screen.queueAddObject(objectToSpawn);
            shouldRemove = true;
        }
    }
    ...
    public void render(Batch batch, float deltaTime) {
        ...
        batch.draw((Texture) screen.game.assets.get("textures/game/warning_circle.png"), x - factor * width / 2, y - factor * height / 2, width * factor, height * factor);
