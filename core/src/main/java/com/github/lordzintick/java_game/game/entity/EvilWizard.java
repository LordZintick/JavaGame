...
import com.github.lordzintick.java_game.game.proj.Projectile;
import com.github.lordzintick.java_game.game.skill.Behaviour;
import com.github.lordzintick.java_game.game.skill.SkillHelper;
...
public class EvilWizard extends HostileEntity {
    ...
    /**
     * Constructs a new {@link EvilWizard} with the provided spritesheet, which will be split into regions of the provided size
     *
     * @param screen  The {@link AbstractGameScreen} containing this entity
     */
    public EvilWizard(AbstractGameScreen screen, Player player) {
        super(screen, screen.game.getAssetOrThrow("textures/game/mobs/evil_wizard.png"), 6, 12, player);
        this.scale = 8f;
        String textureName = "plasma_bolt";
        int val = screen.game.getRandom().nextInt(7);
    ...
    public void update(float deltaTime) {
        ...
        if (cooldown <= 0) {
            cooldown = 3f;
            screen.game.getAudioSample("shoot");
